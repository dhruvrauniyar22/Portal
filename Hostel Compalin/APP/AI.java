...
public final class AI {
    ...
    // Optional OAuth2 access token (Bearer). Obtain using gcloud or a service-account flow.
    private static final String ACCESS_TOKEN = System.getenv("GEMINI_ACCESS_TOKEN");
    ...
    public static boolean isConfigured() {
        return (API_KEY != null && !API_KEY.isBlank()) || (ACCESS_TOKEN != null && !ACCESS_TOKEN.isBlank());
    }
    ...
    public static String askGemini(String prompt) {
        ...
        try {
            String base = "https://generativelanguage.googleapis.com/v1beta/models/" + MODEL + ":generateContent";
            URI endpoint = URI.create(base + (API_KEY != null && !API_KEY.isBlank() ? ("?key=" + API_KEY) : ""));
            connection = (HttpURLConnection) endpoint.toURL().openConnection();
            // If ACCESS_TOKEN is provided prefer Authorization header and omit API key in URL.
    ...
    public static void main(String[] args) {
        System.out.println("GEMINI_API_KEY set: " + isConfigured());
        if (!isConfigured()) {
            System.out.println("Set the GEMINI_API_KEY environment variable to call the API.");
            return;
        }
        // Example prompt — network call will be attempted if API key present.
        String prompt = "Say hello from Gemini.";
